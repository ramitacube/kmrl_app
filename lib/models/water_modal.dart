...
class WaterModel {
  ...
  late final List<WaterList> data;

  WaterModel.fromJson(Map<String, dynamic> json) {
    data = List.from(json['data']).map((e) => WaterList.fromJson(e)).toList();
  }
...
class WaterList {
  WaterList({required this.name, required this.current_reading});
  ...
  WaterList.fromJson(Map<String, dynamic> json) {
    name = json['name'];
